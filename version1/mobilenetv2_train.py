...
#
# HOW: We do a 2D grid search over (downy_thresh × other_thresh) on the
# validation set, maximizing macro F1 across all 3 classes.
# Priority order: downy_mildew first (highest risk), then other_diseases,
# then healthy (only predict healthy if neither disease threshold is met).
# ==========================================
...
def predict_with_thresholds(scores_array, downy_thresh, other_thresh):
    """
    Priority-based classification using per-class thresholds.
    Order: downy_mildew → other_diseases → healthy
    Downy mildew gets first priority as it is the primary target disease.
    """
    preds = []
    for scores in scores_array:
        if scores[DOWNY_CLASS_INDEX] >= downy_thresh:
            preds.append(DOWNY_CLASS_INDEX)
        elif scores[OTHER_CLASS_INDEX] >= other_thresh:
            preds.append(OTHER_CLASS_INDEX)
        else:
            preds.append(1)  # healthy (index 1)
    return np.array(preds)


best_macro_f1      = 0.0
best_downy_threshold = 0.50
best_other_threshold = 0.50

# Coarse grid search — 2D
print("Running 2D threshold grid search (coarse)...")
for downy_t in np.arange(0.20, 0.75, 0.05):
    for other_t in np.arange(0.25, 0.75, 0.05):
        preds    = predict_with_thresholds(val_y_score, downy_t, other_t)
        macro_f1 = f1_score(val_y_true, preds, average='macro', zero_division=0)
        if macro_f1 > best_macro_f1:
            best_macro_f1        = macro_f1
            best_downy_threshold = downy_t
            best_other_threshold = other_t

# Fine search around the best point
print("Fine-tuning around best point...")
for downy_t in np.arange(max(0.10, best_downy_threshold - 0.06),
                          best_downy_threshold + 0.07, 0.01):
    for other_t in np.arange(max(0.10, best_other_threshold - 0.06),
                              best_other_threshold + 0.07, 0.01):
        preds    = predict_with_thresholds(val_y_score, downy_t, other_t)
        macro_f1 = f1_score(val_y_true, preds, average='macro', zero_division=0)
        if macro_f1 > best_macro_f1:
            best_macro_f1        = macro_f1
...
print(f"  Downy threshold:  {best_downy_threshold:.2f}")
print(f"  Other threshold:  {best_other_threshold:.2f}")

# Per-class breakdown with optimized thresholds
opt_preds = predict_with_thresholds(val_y_score, best_downy_threshold, best_other_threshold)
...
# Save threshold config for Flask app
threshold_config = {
    "downy_threshold":    round(float(best_downy_threshold), 2),
    "other_threshold":    round(float(best_other_threshold), 2),
    "downy_class_index":  DOWNY_CLASS_INDEX,
    "other_class_index":  OTHER_CLASS_INDEX,
    "note": (
        "Priority: if downy_mildew score >= downy_threshold → downy_mildew. "
        "Elif other_diseases score >= other_threshold → other_diseases. "
        "Else → healthy."
    )
}
...
y_pred = predict_with_thresholds(y_score, best_downy_threshold, best_other_threshold)
...
print(f"\n--- Threshold-Optimized (downy={best_downy_threshold:.2f}, other={best_other_threshold:.2f}) ---")
