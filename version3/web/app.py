...
#   2. healthy       : second priority — predicted when model is confidently healthy.
#   3. other_diseases: unconditional last-resort fallback (no threshold).
#                      Catches anything that isn't clearly downy or healthy.
# ─────────────────────────────────────────────────────────────────────────────
...
if os.path.exists(_THRESHOLD_CONFIG_PATH):
    with open(_THRESHOLD_CONFIG_PATH) as f:
        _thresh = json.load(f)
    DOWNY_THRESHOLD   = _thresh.get("downy_threshold",   0.55)
    HEALTHY_THRESHOLD = _thresh.get("healthy_threshold", 0.55)
    ...
    print(f"Loaded thresholds — Downy: {DOWNY_THRESHOLD}, Healthy: {HEALTHY_THRESHOLD}")
else:
    # Fallback defaults if config not found (match training minimums)
    DOWNY_THRESHOLD   = 0.55
...
def classify_with_thresholds(preds):
    ...
    Priority order: downy_mildew → healthy → other_diseases
      1. downy_mildew score  >= DOWNY_THRESHOLD   → downy_mildew
      2. healthy score       >= HEALTHY_THRESHOLD  → healthy
...
def predict():
    ...
    # Use threshold-based classification (priority: downy → healthy → other)
    ...
    print(f"RAW argmax: {raw_class} ({raw_conf:.1%}) | "
          ...
          f"Other={preds[OTHER_IDX]:.1%} (fallback) | "
